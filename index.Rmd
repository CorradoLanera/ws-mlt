...
---------------------|------------
Learning Checkers    | .orange[Playing checkers]
Handwriting recognition | .orange[Recognizing/Classifying handwritten words/numbers within images]
Self-driving car | .orange[Driving from A to B]
Diseases extraction from EHR | .orange[classify EHR by the disease reported (in free-text natural language)] 
Describe patient movement in bed | .orange[At any given time provide position and dynamics of patients]
...
---------------------|------------|-------------------
Learning Checkers    | Playing checkers | .orange[% games won ]
Handwriting recognition | Recognizing/Classifying handwritten words/numbers within images | .orange[% correctly classified words ]
Self-driving car | Driving from A to B | .orange[Average distance traveled before an error (as judged by humans)]
Diseases extraction from EHR | classify EHR by the disease reported (in free-text natural language) | .orange[% of EHR correctly classified]
Describe patient movement in bed | At any given time provide position and dynamics of patients | .orange[% average error in position or dynamics]
...
---------------------|------------|-------------------|-----------------
Learning Checkers    | Playing checkers | % games won | .orange[playing (against itself)]
Handwriting recognition | Recognizing/Classifying handwritten words/numbers within images | % correctly classified words | .orange[process data sets of handwritten words with given classification]
Self-driving car | Driving from A to B | Average distance traveled before an error (as judged by humans) | .orange[Sequence of videos, still images, and steering commands recorded while observing a human driver]
Diseases extraction from EHR | classify EHR by the disease reported (in free-text natural language) | % of EHR correctly classified | .orange[process EHRs with given classification]
Describe patient movement in bed | At any given time provide position and dynamics of patients | % average error in position or dynamics | .orange[time-series of patient kinetic measures taken from wearable devices and bed weight sensors, and position and dynamics collected by videos recorded observing inbed patients]
...
---
# .orange[Machine Learning in action]
  
```{r, echo=FALSE, message=FALSE, warning=FALSE,  out.width="40%"}
myimages <- list.files(
  "img/pendulum/", pattern = ".png", full.names = TRUE
)
include_graphics(myimages)
```

$$f(\rm state,\ action) = \rm next\ state$$
...
.pull-left[
Task: maintain the bar vertically
  - Outcome: Action (1 var): (new) chart .orange[signed velocity]

Performance: .orange[distance] from vertical
]

.pull-right[
Experience: State (4 vars):
  - cart .orange[location]
  - cart .orange[velocity]
  - .orange[angle]
  - .orange[angular velocity]
]
...
<small>
<br>
(credits: PILCO - Probabilistic Inference for Learning COntrol, University of Cambridge)
